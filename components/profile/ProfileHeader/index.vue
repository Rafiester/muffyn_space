      ...
      >
        ...
        <div class="flex gap-4 items-center justify-center">
          <template v-if="profile.socials">
            <a
              v-for="[cardKey, cardData] in Object.entries(profile.socials).filter(([k, v]) => k.startsWith('card') && typeof v === 'object' && v.url)"
              :key="cardKey"
              :href="(cardData as SocialCard).url"
              target="_blank"
              rel="noopener noreferrer"
              :aria-label="`${profile.name}'s ${cardKey}`"
              :class="[
                'p-2.5 transition-all duration-300 w-10 h-10 flex items-center justify-center',
                isRetro 
            ...
            >
              ...
              <img 
                v-if="(cardData as SocialCard).icon.startsWith('data:image') || (cardData as SocialCard).icon.startsWith('http')" 
                :src="(cardData as SocialCard).icon" 
                class="w-5 h-5 object-contain" 
                alt="Social Icon"
              />
              
              <!-- Built-in GitHub -->
              <svg v-else-if="(cardData as SocialCard).icon === 'github'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" stroke-width="2" fill="none" stroke="currentColor">
                <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
              </svg>
              
              <!-- Built-in Twitter -->
              <svg v-else-if="(cardData as SocialCard).icon === 'twitter'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" stroke-width="2" fill="none" stroke="currentColor">
                <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
              </svg>

              <!-- Built-in LinkedIn -->
              <svg v-else-if="(cardData as SocialCard).icon === 'linkedin'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" stroke-width="2" fill="none" stroke="currentColor">
              ...
              <!-- Built-in Email -->
              <svg v-else-if="(cardData as SocialCard).icon === 'email'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" stroke-width="2" fill="none" stroke="currentColor">
                <rect width="20" height="16" x="2" y="4" rx="2" />
                <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
              </svg>

              <!-- Fallback Globe Icon for anything else -->
